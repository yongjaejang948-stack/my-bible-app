...
# ====================== GitHub bible.json 자동 로드 ======================
@st.cache_data
def load_bible_from_github():
    """GitHub 저장소에 있는 bible.json을 자동으로 로드"""
    possible_paths = [
        "bible.json",                    # 루트 폴더
        "data/bible.json",               # data 폴더
        "assets/bible.json",             # assets 폴더
        "static/bible.json",             # static 폴더
        "my-bible-app/bible.json",       # 하위 폴더
    ]
    ...
    st.info("해결 방법: bible.json 파일을 저장소 루트 폴더에 두세요.")
    return []

def parse_bible_data(data):
    """다양한 JSON 구조를 표준 형태로 변환"""
    formatted = []
...
with st.sidebar:
    ...
    if st.session_state.bible_data:
        ...
        st.info("GitHub 저장소 루트에 bible.json을 두세요")
